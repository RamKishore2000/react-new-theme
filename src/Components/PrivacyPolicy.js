...
const renderContent = (data) => {
  if (!data || data.length === 0) return <p className="text-muted">No privacy policy available.</p>;
...
export default function PrivacyPolicy() {
  const [content, setContent] = useState(() => {
    const cached = localStorage.getItem("privacy_policy_data");
    ...
    const fetchPolicy = async () => {
      try {
        ...
        if (res.data?.status === "success") {
          ...
          localStorage.setItem("privacy_policy_data", JSON.stringify(res.data.response));
        }
      } catch (err) {
